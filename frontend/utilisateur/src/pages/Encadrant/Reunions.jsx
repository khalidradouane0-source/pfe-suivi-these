...
const EncReunions = () => {
  ...
  const [newReunion, setNewReunion] = useState({
    titre: '', date_reunion: '', heure: '', participants: '', statut: 'planifiee', doctorant_id: ''
  })
  ...
  const addReunion = () => {
    ...
    if (newReunion.doctorant_id === 'tous') {
      Promise.all(doctorants.map(doc =>
        fetch('http://localhost:3001/api/reunions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...newReunion, doctorant_id: doc.id })
        }).then(res => res.json())
        .then(data => ({
          id: data.id,
          titre: newReunion.titre,
          date_reunion: newReunion.date_reunion,
          heure: newReunion.heure,
          participants: newReunion.participants,
          statut: newReunion.statut,
          nom: doc.nom,
          prenom: doc.prenom
        }))
      )).then(newReunions => {
        setReunions(prev => [...prev, ...newReunions]);
        setNewReunion({ titre: '', date_reunion: '', heure: '', participants: '', statut: 'planifiee', doctorant_id: '' });
        setShowModal(false);
      });
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Ex: Dr. Mohammed, Dr. Rachida"
                  value={newReunion.participants}
              ...
              <div>
                ...
                <select
                  value={newReunion.statut}
                  onChange={e => setNewReunion({ ...newReunion, statut: e.target.value })}
                  className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                >
