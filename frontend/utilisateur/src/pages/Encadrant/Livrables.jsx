...
import {
  FileTextIcon, CheckCircleIcon, AlertCircleIcon, ClockIcon,
  DownloadIcon, MessageSquareIcon, ChevronDownIcon, ChevronUpIcon
} from 'lucide-react'
...
const EncLivrables = () => {
  ...
  const updateStatut = (id, statut) => {
    const livrable = livrables.find(l => l.id === id)
    fetch(`http://localhost:3001/api/livrables/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ statut, encadrant_nom: `${user.prenom} ${user.nom}`, doctorant_id: livrable?.doctorant_id || null })
    })
    ...
    <div className="p-6 space-y-5 max-w-5xl mx-auto">
      ...
      {livrables.filter(l => l.statut === 'soumis').length > 0 && (
        <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 border border-blue-200 rounded-xl text-sm text-blue-700">
          ...
          <span><strong>{livrables.filter(l => l.statut === 'soumis').length}</strong> livrable(s) en attente</span>
        </div>
      )}
              ...
              <div className="flex items-center gap-4 px-4 py-4">
                ...
                {l.fichier_url ? (
                  <a href={`http://localhost:3001${l.fichier_url}`} target="_blank" rel="noreferrer"
                    className="flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-50 text-slate-600 text-xs hover:bg-indigo-50 hover:text-indigo-600 transition border border-slate-200">
                ...
                <div className="flex gap-2">
                  {l.statut === 'soumis' && (
                    <>
                      <button onClick={() => updateStatut(l.id, 'valide')}
                        className="px-2 py-1 rounded-lg bg-emerald-50 text-emerald-700 text-xs font-medium hover:bg-emerald-100 border border-emerald-200">
                        ✅ Valider
                      </button>
                      <button onClick={() => updateStatut(l.id, 'a_corriger')}
                        className="px-2 py-1 rounded-lg bg-amber-50 text-amber-700 text-xs font-medium hover:bg-amber-100 border border-amber-200">
                        ✏️ Corriger
                      </button>
                    </>
                  )}
                  ...
                  {l.statut === 'valide' && <span className="text-xs text-emerald-600 font-medium">✅ Validé</span>}
                </div>
              </div>
              ...
              {isOpen && (
                <div className="border-t border-slate-100 px-4 py-3 bg-slate-50 space-y-3">
                  ...
                  <div className="space-y-2">
                    {comments.map(c => (
                      <div key={c.id} className={`flex gap-2 ${c.auteur_role === 'encadrant' ? 'flex-row-reverse' : ''}`}>
                        <div className={`max-w-xs px-3 py-2 rounded-xl text-xs ${
                          c.auteur_role === 'encadrant'
                            ? 'bg-teal-600 text-white'
                            : 'bg-white border border-slate-200 text-slate-700'
                        }`}>
                          <p className={`text-[10px] font-medium mb-0.5 ${c.auteur_role === 'encadrant' ? 'text-teal-200' : 'text-slate-400'}`}>
                            {c.auteur_nom}
                          </p>
                          <p>{c.contenu}</p>
                          <p className={`text-[10px] mt-0.5 ${c.auteur_role === 'encadrant' ? 'text-teal-200' : 'text-slate-400'}`}>
