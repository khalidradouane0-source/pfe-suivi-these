  ...
  return (
    <div className="p-6 max-w-5xl mx-auto h-[calc(100vh-120px)]">
      ...
      <div className="flex items-center gap-3 mb-5">
        <MessageSquareIcon className="w-6 h-6 text-sky-600" />
        <div>
          <h1 className="text-2xl font-semibold text-slate-800">Messages</h1>
          <p className="text-sm text-slate-500">Communication avec les encadrants</p>
        </div>
      </div>
      ...
      <div className="flex gap-4 h-[calc(100%-80px)]">
        ...
        <div className="w-72 bg-white border border-slate-100 rounded-xl shadow-sm overflow-hidden flex-shrink-0">
          <div className="px-4 py-3 border-b border-slate-100">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest">Encadrants</p>
          </div>
            ...
            {contacts.map(c => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-left transition border-b border-slate-50 last:border-0 ${
                  selectedContact?.id === c.id ? 'bg-sky-50' : 'hover:bg-slate-50'
                }`}
              >
                <div className="w-9 h-9 rounded-xl bg-indigo-50 flex items-center justify-center flex-shrink-0">
                  <span className="text-indigo-600 font-semibold text-sm">{c.prenom?.charAt(0).toUpperCase()}</span>
                </div>
            ...
            <>
              <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-indigo-50 flex items-center justify-center">
                  <span className="text-indigo-600 font-semibold text-sm">{selectedContact.prenom?.charAt(0).toUpperCase()}</span>
                </div>
                <div>
                  ...
                  return (
                    <div key={m.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-xs px-3 py-2 rounded-xl text-sm ${
                        isMine ? 'bg-sky-600 text-white' : 'bg-slate-100 text-slate-700'
                      }`}>
                        <p>{m.contenu}</p>
