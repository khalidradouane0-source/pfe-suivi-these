...
const SideBar = () => {
  ...
  const roleLabels = {
    doctorant:      { title: "Doctorant",       subtitle: "Suivi de thèse" },
    encadrant:      { title: "Encadrant",        subtitle: "Encadrement doctoral" },
    "co-encadrant": { title: "Co-Encadrant",     subtitle: "Suivi scientifique" },
    responsable:    { title: "Responsable",      subtitle: "Suivi global des doctorants" },
    admin:          { title: "Administrateur",   subtitle: "Gestion de la plateforme" },
  }

  const navItemsByRole = {
    doctorant: [
      { name: "Dashboard",  href: "/doctorant",            icon: LayoutGridIcon },
      { name: "Objectifs",  href: "/doctorant/objectifs",  icon: ClipboardListIcon },
    ...
    encadrant: [
      ...
      { name: "Objectifs",  href: "/encadrant/objectifs",  icon: ClipboardListIcon },
      { name: "Reunions",   href: "/encadrant/reunions",   icon: CalendarIcon },
      { name: "Livrables",  href: "/encadrant/livrables",  icon: FileTextIcon },
      { name: "Messages", href: "/encadrant/messages", icon: MessageSquareIcon },

    ],
    "co-encadrant": [
      { name: "Dashboard",  href: "/co-encadrant",            icon: LayoutGridIcon },
      { name: "Doctorants", href: "/co-encadrant/doctorants", icon: UserIcon },
      { name: "Reunions",   href: "/co-encadrant/reunions",   icon: CalendarIcon },
      { name: "Livrables",  href: "/co-encadrant/livrables",  icon: FileTextIcon },
      { name: "Messages", href: "/co-encadrant/messages", icon: MessageSquareIcon },
    ],
    responsable: [
  ...
  const sidebarcontent = (
    <>
        ...
        <div className='mx-3 mt-4 mb-2 p-4 rounded-xl bg-white/5 border border-white/10'>
          <div className='flex items-center gap-3'>
            ...
            <div className='min-w-0'>
              ...
              <p className='text-xs text-slate-400 truncate'>{roleLabels[role]?.subtitle}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className='px-5 pt-5 pb-2'>
        <p className='text-[11px] font-semibold uppercase tracking-[0.15em] text-slate-500'>Navigation</p>
      </div>
          ...
          return (
            <Link
              ...
              className={`group flex items-center gap-4 px-4 py-3 rounded-xl text-[14px] font-medium transition-all duration-200 relative ${
                isActive ? "bg-white/10 text-white shadow-sm" : "text-slate-300 hover:text-white hover:bg-white/5"
              }`}
            >
              <item.icon className={`w-[18px] h-[18px] ${isActive ? "text-white" : "text-slate-400 group-hover:text-white"}`} />
              <span className='flex-1'>{item.name}</span>
              {isActive && <div className="w-2 h-2 rounded-full bg-white opacity-70"/>}
              {isActive && <ChevronRightIcon className='w-4 h-4 text-white/70'/>}
            </Link>
          )
        })}
      </div>
  ...
  return (
    <>
      ...
      <aside className='hidden lg:flex flex-col h-full w-[260px] bg-gradient-to-b from-indigo-900 via-indigo-900 to-slate-900 text-white border-r border-white/10'>
        {sidebarcontent}
      </aside>

      <aside className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-gradient-to-b from-indigo-900 to-slate-900 text-white z-50 flex flex-col transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        {sidebarcontent}
      </aside>
    </>
  )
